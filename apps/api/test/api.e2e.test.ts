...
describe('인증 · 테넌트 격리', () => {
  ...
  it('다른 테넌트의 근무표는 조회되지 않는다', async () => {
    // roster_ward3_202608은 tenant_seoul 소유다.
    await api()
      .get('/api/v1/rosters/roster_ward3_202608')
  ...
  it('다른 테넌트의 근태 기록은 보이지 않는다', async () => {
    ...
    const seoul = await api()
      ...
      .set('Authorization', SEOUL_NURSE)
      .expect(200);
    expect(seoul.body.data.every((r: { memberId: string }) => r.memberId !== 'mem_busan')).toBe(
      true,
    );
  });
});
...
describe('출퇴근 기록', () => {
  it('병원 WiFi가 확인되면 HIGH 신뢰도로 승인된다', async () => {
    ...
    expect(res.body.data.worksiteId).toBe('ws_seoul_main');
  });
  ...
  it('지오펜스 밖이면 422지만 기록은 저장된다 — 막다른 길을 만들지 않는다', async () => {
    ...
    // 오류 응답에도 data가 함께 온다. 앱이 "승인 대기"를 정확히 표시할 수 있어야 한다.
  ...
  it('일반 구성원은 처리할 수 없다', async () => {
    const created = await api()
      ...
      .send({
        ...
        device: { deviceId: 'device_abc' },
        clientNonce: 'pending-for-auth-test',
      })
      .expect(422);

    const res = await api()
      .post(`/api/v1/attendance/pending-reviews/${created.body.data.id}/resolve`)
      .set('Authorization', SEOUL_NURSE)
...
describe('근무표 규칙 평가', () => {
  ...
  it('특례 서면합의가 적용된 사업장이라 52시간 규칙 대신 11시간 휴식 규칙이 걸린다', async () => {
    const res = await api()
      .get('/api/v1/rosters/roster_ward3_202608')
      .set('Authorization', SEOUL_MANAGER)
      .expect(200);
  ...
  it('BLOCK 위반이 있으면 확정을 막고 위반 내역을 돌려준다', async () => {
    const res = await api()
      .post('/api/v1/rosters/roster_ward3_202608/publish')
      .set('Authorization', SEOUL_MANAGER)
      .send({})
      .expect(409);
...
describe('휴가', () => {
  ...
  it('취소하면 잔액이 복원된다', async () => {
    ...
    const created = await api()
      .post('/api/v1/leaves/requests')
      .set('Authorization', SEOUL_NURSE)
      .send({ leaveTypeId: 'lt_annual', startDate: '2026-08-20' })
      .expect(201);
  ...
  it('반차는 0.5일만 차감한다', async () => {
    const res = await api()
      .post('/api/v1/leaves/requests')
      .set('Authorization', SEOUL_NURSE)
      .send({ leaveTypeId: 'lt_half', startDate: '2026-08-25', units: 50 })
      .expect(201);
  ...
  it('같은 부서 동료의 휴가가 겹치면 경고한다', async () => {
    await api()
      .post('/api/v1/leaves/requests')
      .set('Authorization', SEOUL_MANAGER)
      .send({ leaveTypeId: 'lt_annual', startDate: '2026-08-26' })
      .expect(201);
  ...
  it('잔액을 차감하지 않는 휴가 종류는 차감 내역이 없다', async () => {
    const res = await api()
      .post('/api/v1/leaves/requests')
      .set('Authorization', SEOUL_NURSE)
      .send({ leaveTypeId: 'lt_sick', startDate: '2026-11-02' })
      .expect(201);
